...
router.post("/register", async (req, res) => {
  try {
    ...
    if (password.length < 5) {
      return res
        .status(400)
        .json({ msg: "Passowrd needs to be at least 5 characters long." });
    }
    if (password !== passwordCheck) {
      return res
        .status(400)
        .json({ msg: "Enter the same passowrd twice for verification." });
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ msg: "Not acocunt with this email exists." });
    }
